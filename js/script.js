...
fetch('./json/platos.json')
    ...
    .then((platos) => {
        ...
        platos.forEach(plato => {
                divPlatos.innerHTML += 
                `
                <p class= "copy1"> ${plato.nombre} </p>
                <p class= "copy2">En su preparación encontramos ${plato.ingredientes} </p>
                <p class= "copy3">Es un plato común de ${plato.origen} </p>
                <img class="img-platos" src="../img/${plato.img} " alt="platos de comida por pais">
                `})
        })
